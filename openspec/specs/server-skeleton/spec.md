...
Go 单体服务骨架：进程启动、配置加载、结构化日志（trace_id）、MySQL/Redis 连接、migration 自动执行、健康检查端点。为 W2+ 业务功能提供固定分层地基（apps/server，分层见 LLM_DEV_GUIDE.md §3.3）。
...
服务启动时 SHALL 初始化 go-redis 客户端并对 Redis 执行 PING；连接失败 MUST 以 error 级别日志记录（进程不因 Redis 不可用退出，W2 起按业务决定失败策略）。
